...
When('informa os dados obrigatorios', async function (this: ScenarioWorld) {
  // Nota: gera dados únicos por execução para evitar colisões em ambiente compartilhado.
...
Given('que existe um funcionario cadastrado', async function (this: ScenarioWorld) {
  // Nota: reaproveita massa de apoio criada no hook para reduzir custo e tempo de setup.
...
Then(
  'os dados do funcionario devem ser atualizados no sistema',
...
Then('o acesso associado ao funcionario deve ser revogado', async function (this: ScenarioWorld) {
  ...
  const credentials = getEmployeeLoginCredentials(this);
  // Nota: garante que o admin esteja deslogado antes de tentar login do funcionário deletado
